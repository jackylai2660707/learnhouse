...
def upgrade() -> None:
    # --- Enum additions ---------------------------------------------------
    # PostgreSQL 16 supports transactional enum additions. Keep these changes
    # in Alembic's transaction so a later failure cannot leave a partial schema.
    ...
    op.execute(
        "ALTER TYPE assignmenttasktypeenum ADD VALUE IF NOT EXISTS 'SHORT_ANSWER'"
    )
    op.execute(
        "ALTER TYPE assignmenttasktypeenum ADD VALUE IF NOT EXISTS 'NUMBER_ANSWER'"
    )
